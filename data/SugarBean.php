...
//    	require_once('data/Link.php');
                ...
                if (isset($list_column[0]))
                {
                    ...
                    if (isset($bean_queried->field_defs[$list_column_name]))
                    {
                        ...
                        if ($this->db->dbType == 'mssql'
                                && $source != 'non-db'
                                && in_array(
                                        $this->db->getHelper()->getColumnType($this->db->getHelper()->getFieldType($bean_queried->field_defs[$list_column_name])), array('ntext', 'text', 'image')
                                )
                        )
                        {
                        ...
                        if ($this->db->dbType == 'oci8'
                                && $source != 'non-db'
                                && in_array(
                                        $this->db->getHelper()->getColumnType($this->db->getHelper()->getFieldType($bean_queried->field_defs[$list_column_name])), array('clob')
                                )
                        )
                        {
    ...
    function create_new_list_query($order_by, $where, $filter = array(), $params = array(), $show_deleted = 0, $join_type = '', $return_array = false, $parentbean = null, $singleSelect = false)
    {
        ...
        if (!$this->disable_row_level_security)
        {
            ...
            if (!$singleSelect)
                $this->add_team_security_where_clause($ret_array['secondary_from']);
        }
        if ($custom_join)
        {
            ...
            if ($data['type'] == 'relate' && isset($data['link']))
            {
                ...
                if (!empty($this->$data['link']))
                {
                    ...
                    if ($join['type'] == 'many-to-many')
                    {
                        ...
                        $count_used = 0;
                        if ($this->db->dbType != 'mysql')
                        {//bug 26801, these codes are just used to duplicate rel_key in the select sql, or it will throw error in MSSQL and Oracle.
...
//					$this->format_field($value);
    ...
    private function _sendNotifications($check_notify)
    {
        ...
        { // cn: bug 5795 - no invites sent to Contacts, and also bug 25995, in workflow, it will set the notify_on_save=true.
