...
function clean_xss($str, $cleanImg = true)
{
    ...
    $matches = array_merge(
            xss_check_pattern($tag_regex, $str), xss_check_pattern($javascript_regex, $str), xss_check_pattern($attribute_regex, $str)
    );
...
{
    return SugarThemeRegistry::current()->getImage(basename($image), $other_attributes, empty($width) ? null : $width, empty($height) ? null : $height
    );
}
