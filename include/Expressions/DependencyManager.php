...
class DependencyManager
{
            ...
            if (isset($def['calculated']) && $def['calculated'] && !empty($def['formula']))
            {
                        ...
                        (!is_string($def['enforced']) || strtolower($def['enforced']) !== "false"))
                {
                    ...
                    if ($includeReadOnly)
                    {
                        ...
                        $readOnlyDep->addAction(ActionFactory::getNewAction('ReadOnly', array('target' => $field,
                                    'value' => 'true')));
        ...
        foreach ($fields as $field => $def)
        {
            if (!empty($def ['dependency']))
            {
                ...
                foreach ($def ['dependency'] as $depdef)
                {
                    ...
                    $dep->setTrigger(new Trigger('true', $triggerFields));
                    $dep->addAction(ActionFactory::getNewAction('SetVisibility', array('target' => $field, 'value' => $depdef ['action'])));
                    $dep->setFireOnLoad(true);
                    $deps[] = $dep;
                }
            }
    ...
    static function getDependenciesForFields($fields, $view = "")
    {
        ...
        {
            return array_merge(
                            self::getCalculatedFieldDependencies($fields), self::getDependentFieldDependencies($fields), self::getDropDownDependencies($fields));
        }
    }
